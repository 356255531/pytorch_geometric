...
class GATSolver(BaseSolver):
    def __init__(self, GAT, dataset_args, model_args, train_args):
        super(GATSolver, self).__init__(GAT, dataset_args, model_args, train_args)

    def prepare_model_input(self, data, if_use_features=False):
        edge_index_np = data.edge_index_nps[0]['user2item']
...
if __name__ == '__main__':
    solver = GATSolver(GAT, dataset_args, model_args, train_args)
    solver.run()
