...
def single_run_with_kg(run, model, opt, loss_func, dataset_args, train_args):
    torch.random.manual_seed(run + 2019)

    train_args['root'] = osp.join(train_args['data_folder'], 'seed{}'.format(run + 2019))
    data = MovieLens(**dataset_args).data.to(train_args['device'])
    edge_iter, train_rating_edge_iter, test_rating_edge_iter = get_iters(data, batch_size=train_args['batch_size'])


    filename = 'run{}_seed{}_hs{}_lr{}_core{}'
    debug = '' if not train_args['debug'] else '_debug{}'.format(train_args['debug'])
    logger = SummaryWriter(osp.join(train_args['logger_folder'], filename + debug))
    ...
    weights_path = osp.join(weights_folder, filename + '_{}'.format(epoch) + debug + '.pkl')
