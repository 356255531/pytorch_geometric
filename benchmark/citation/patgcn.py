...
class Net(torch.nn.Module):
    def __init__(self, dataset):
        ...
        self.att_block = ConvEncoder(2, 2)
    ...
    def path_transform(self, meta_path_edge_indicis, att_block, meta_path_edge_values=None):
        ...
        return sparse_adapted_adj.indices(), sparse_adapted_adj.values() / len(meta_path_edge_indicis)
    ...
    def forward(self, data):
        ...
        x = F.relu(self.conv1(x, meta_edge_index, edge_weight=edge_weight))
        x = F.dropout(x, p=args.dropout, training=self.training)
        x = self.conv2(x, meta_edge_index, edge_weight=edge_weight)
